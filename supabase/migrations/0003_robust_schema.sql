...
create table if not exists public.departments (
  ...
  code text not null unique, -- Ensure code is unique
  description text,
  head_id uuid, 
  created_at timestamp with time zone default timezone('utc'::text, now()) not null
);
...
-- Insert default departments SAFELY (Checking by name AND code)
insert into public.departments (name, code, description)
select 'Mayor''s Office', 'MO', 'Office of the Municipal Mayor'
where not exists (select 1 from public.departments where name = 'Mayor''s Office' or code = 'MO');

insert into public.departments (name, code, description)
select 'Human Resources', 'HR', 'Human Resource Management Office'
where not exists (select 1 from public.departments where name = 'Human Resources' or code = 'HR');

insert into public.departments (name, code, description)
select 'Engineering', 'ENG', 'Municipal Engineering Office'
where not exists (select 1 from public.departments where name = 'Engineering' or code = 'ENG');

insert into public.departments (name, code, description)
select 'Health', 'MHO', 'Municipal Health Office'
where not exists (select 1 from public.departments where name = 'Health' or code = 'MHO');

insert into public.departments (name, code, description)
select 'Social Welfare', 'MSWDO', 'Municipal Social Welfare and Development Office'
where not exists (select 1 from public.departments where name = 'Social Welfare' or code = 'MSWDO');
